    ...
    fn run(
        ...
        if let Some(cursor) = screen_to_world(&cameras, &input, &dimension, &transformations) {
            self.point = match self.point {
                ...
                None => {
                    let selected = (&sprites, &transformations, &cards, &entities)
                        .join()
                        .filter(
                            |(sprite, sprite_transform, card, entity)| match card.state {
                                CardState::TableauCovered => false,
                                _ => {
                                    if let Some(sprite_sheet) =
                                        sheet_storage.get(&sprite.sprite_sheet)
                                    {
                        ...
                        .map(|(sprite, sprite_transform, card, entity)| {
                            (entity, *sprite_transform.translation())
                        })
